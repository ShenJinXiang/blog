...
category: Python
tags: Python
description: 字典（dictionary）是Python中常见得一种数据类型，和list有点类似，主要的差别在于list中的元素是根据偏移来存取的，而字典中的元素是通过键来存取的
...
print(d)
# {0: 0, 1: 2, 2: 4, 3: 6, 4: 8, 5: 10, 6: 12, 7: 14, 8: 16, 9: 18} </code>
</pre>
<h4>访问dict</h4>
<pre class='line-numbers language-python'>
<code># 通过D[key]的形式获取value
d = {'name' : 'Bob', 'age' : 18}
print(d['name'])			# Bob
print(d['age'])			# 18
...
print(d.get('address', 'china'))		# china </code>
</pre>
<h4>添加或修改dict</h4>
<pre class="line-numbers language-python">
<code>d = {'name' : 'Bob', 'age' : 18}
print(d)	# {'age': 18, 'name': 'Bob'}
# 添加新项
d['sex'] = '男'
...
# 修改dic中的项
d['age'] = 19
print(d)	# {'age': 19, 'sex': '男', 'name': 'Bob'} </code>
</pre>
<h4>删除dict中的项</h4>
<pre class='line-numbers language-python'>
<code># 使用del
...
v = d.pop('age')
print(v)		# 18
print(d)		# {'name': 'Bob'} </code>
</pre>
<h4>遍历dict</h4>
<pre class='line-numbers language-python'>
<code># 与遍历list类似，使用for循环实现
d = {'name' : 'Bob', 'age' : 18, 'sex' : '男'}
for key in d :
	print(key, '-->', d[key])
# age --> 18
...
print(d1)			# {'address': 'china', 'name': 'Tom'}
d1.update(d)		# {'address': 'china', 'sex': '男', 'age': 18, 'name': 'Tom'}</code>
</pre>
